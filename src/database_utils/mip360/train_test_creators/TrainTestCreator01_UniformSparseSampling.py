...
def sample_sparse_train_frames(scene_dirpath: Path, frame_nums: list, num_frames: int):
    if num_frames != -1:
        selected_frame_nums = numpy.fromfile(scene_dirpath / f'sparse_{num_frames}.txt', dtype=numpy.uint8, sep='\n')
    else:
...
def create_train_test_set(configs: dict):
    ...
    for scene_name in scene_names:
        ...
        train_frame_nums = sample_sparse_train_frames(scenes_dirpath / scene_name, frame_nums, num_train_frames)
        test_frame_nums = [frame_num for frame_num in frame_nums if frame_num not in train_frame_nums]
        val_frame_nums = test_frame_nums
